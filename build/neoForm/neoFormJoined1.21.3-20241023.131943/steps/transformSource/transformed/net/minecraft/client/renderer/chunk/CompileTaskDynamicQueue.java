...
public class CompileTaskDynamicQueue {
    ...
    @Nullable
    public synchronized SectionRenderDispatcher.RenderSection.CompileTask poll(Vec3 cameraPosition) {
        int i = -1;
        int j = -1;
        ...
        while (listiterator.hasNext()) {
            ...
            if (sectionrenderdispatcher$rendersection$compiletask.isCancelled.get()) {
                ...
                double d2 = sectionrenderdispatcher$rendersection$compiletask.getOrigin().distToCenterSqr(cameraPosition);
    ...
    @Nullable
    private SectionRenderDispatcher.RenderSection.CompileTask removeTaskByIndex(int index) {
        return index >= 0 ? this.tasks.remove(index) : null;
    }
