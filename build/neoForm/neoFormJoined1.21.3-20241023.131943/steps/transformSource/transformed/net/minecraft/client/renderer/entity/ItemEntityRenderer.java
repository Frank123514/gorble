...
public class ItemEntityRenderer extends EntityRenderer<ItemEntity, ItemEntityRenderState> {
    ...
    public void extractRenderState(ItemEntity entity, ItemEntityRenderState reusedState, float partialTick) {
        ...
        reusedState.ageInTicks = (float)entity.getAge() + partialTick;
    ...
    public void render(ItemEntityRenderState renderState, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        BakedModel bakedmodel = renderState.itemModel;
        if (bakedmodel != null) {
            poseStack.pushPose();
            ItemStack itemstack = renderState.item;
            ...
            float f1 = shouldBob ? Mth.sin(renderState.ageInTicks / 10.0F + renderState.bobOffset) * 0.1F + 0.1F : 0;
            float f2 = bakedmodel.getTransforms().getTransform(ItemDisplayContext.GROUND).scale.y();
            poseStack.translate(0.0F, f1 + 0.25F * f2, 0.0F);
            float f3 = ItemEntity.getSpin(renderState.ageInTicks, renderState.bobOffset);
            poseStack.mulPose(Axis.YP.rotation(f3));
            renderMultipleFromCount(this.itemRenderer, poseStack, bufferSource, packedLight, itemstack, bakedmodel, flag, this.random);
            poseStack.popPose();
