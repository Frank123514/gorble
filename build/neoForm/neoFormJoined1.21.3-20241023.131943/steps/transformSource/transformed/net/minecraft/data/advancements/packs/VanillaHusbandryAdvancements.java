...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
    ...
    private static Advancement.Builder addLeashedFrogVariants(
        ...
        BuiltInRegistries.FROG_VARIANT
            .listElements()
            .forEach(
                p_359387_ -> builder.addCriterion(
                        p_359387_.key().location().toString(),
                        PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(
                            ItemPredicate.Builder.item().of(items, Items.LEAD),
                            Optional.of(
                                EntityPredicate.wrap(
                                    EntityPredicate.Builder.entity().of(entityTypeRegistry, EntityType.FROG).subPredicate(EntitySubPredicates.frogVariant(p_359387_))
                                )
                            )
                        )
                    )
    ...
    private static Advancement.Builder addBreedable(
        Advancement.Builder builder, Stream<EntityType<?>> breedableAnimals, HolderGetter<EntityType<?>> entityTypeRegistry, Stream<EntityType<?>> indirectlyBreedableAnimals
    ) {
        breedableAnimals.forEach(
            p_359390_ -> builder.addCriterion(
                    EntityType.getKey((EntityType<?>)p_359390_).toString(),
                    BredAnimalsTrigger.TriggerInstance.bredAnimals(EntityPredicate.Builder.entity().of(entityTypeRegistry, (EntityType<?>)p_359390_))
                )
        );
        indirectlyBreedableAnimals.forEach(
            p_359383_ -> builder.addCriterion(
                    EntityType.getKey((EntityType<?>)p_359383_).toString(),
                    BredAnimalsTrigger.TriggerInstance.bredAnimals(
                        Optional.of(EntityPredicate.Builder.entity().of(entityTypeRegistry, (EntityType<?>)p_359383_).build()),
                        Optional.of(EntityPredicate.Builder.entity().of(entityTypeRegistry, (EntityType<?>)p_359383_).build()),
                        Optional.empty()
                    )
                )
