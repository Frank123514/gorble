...
public class WitchModel extends EntityModel<WitchRenderState> implements HeadedModel, VillagerHeadModel {
    ...
    public void setupAnim(WitchRenderState renderState) {
        ...
        this.rightLeg.xRot = Mth.cos(renderState.walkAnimationPos * 0.6662F) * 1.4F * renderState.walkAnimationSpeed * 0.5F;
        this.leftLeg.xRot = Mth.cos(renderState.walkAnimationPos * 0.6662F + (float) Math.PI) * 1.4F * renderState.walkAnimationSpeed * 0.5F;
        float f = 0.01F * (float)(renderState.entityId % 10);
        this.nose.xRot = Mth.sin(renderState.ageInTicks * f) * 4.5F * (float) (Math.PI / 180.0);
        this.nose.zRot = Mth.cos(renderState.ageInTicks * f) * 2.5F * (float) (Math.PI / 180.0);
        if (renderState.isHoldingItem) {
