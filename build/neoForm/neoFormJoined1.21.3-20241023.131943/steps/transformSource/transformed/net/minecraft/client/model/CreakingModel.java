...
public class CreakingModel extends EntityModel<CreakingRenderState> {
    ...
    public CreakingModel(ModelPart root) {
        super(root);
        ModelPart modelpart = root.getChild("root");
    ...
    public void setupAnim(CreakingRenderState renderState) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        if (renderState.canMove) {
            this.animateWalk(CreakingAnimation.CREAKING_WALK, renderState.walkAnimationPos, renderState.walkAnimationSpeed, 5.5F, 3.0F);
        }

        this.animate(renderState.attackAnimationState, CreakingAnimation.CREAKING_ATTACK, renderState.ageInTicks);
        this.animate(renderState.invulnerabilityAnimationState, CreakingAnimation.CREAKING_INVULNERABLE, renderState.ageInTicks);
    }

    public List<ModelPart> getHeadModelParts(CreakingRenderState renderState) {
        return !renderState.isActive ? NO_PARTS : this.headParts;
    }
}
