...
public class PiglinAi {
    ...
    private static void initIdleActivity(Brain<Piglin> brain) {
        brain.addActivity(
            ...
            ImmutableList.of(
                ...
                StartAttacking.<Piglin>create((p_381522_, p_381523_) -> p_381523_.isAdult(), PiglinAi::findNearestValidAttackTarget),
    ...
    private static void initFightActivity(Piglin piglin, Brain<Piglin> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(
            ...
            ImmutableList.<net.minecraft.world.entity.ai.behavior.BehaviorControl<? super Piglin>>of(
                StopAttackingIfTargetInvalid.<Piglin>create((p_375910_, p_375911_) -> !isNearestValidAttackTarget(p_375910_, piglin, p_375911_)),
    ...
    protected static void pickUpItem(ServerLevel level, Piglin piglin, ItemEntity itemEntity) {
        ...
        if (isLovedItem(itemstack)) {
            ...
            boolean flag = !piglin.equipItemIfPossible(level, itemstack).equals(ItemStack.EMPTY);
            if (!flag) {
    ...
    protected static void stopHoldingOffHandItem(ServerLevel level, Piglin piglin, boolean barter) {
        ...
        if (piglin.isAdult()) {
            ...
            if (barter && flag) {
                ...
                boolean flag1 = !piglin.equipItemIfPossible(level, itemstack).isEmpty();
                if (!flag1) {
                    putInInventory(piglin, itemstack);
                }
            }
        } else {
            boolean flag2 = !piglin.equipItemIfPossible(level, itemstack).isEmpty();
            if (!flag2) {
    ...
    private static boolean isNearestValidAttackTarget(ServerLevel level, Piglin piglin, LivingEntity target) {
        return findNearestValidAttackTarget(level, piglin).filter(p_34887_ -> p_34887_ == target).isPresent();
    }
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel level, Piglin piglin) {
        ...
        if (isNearZombified(piglin)) {
            ...
            if (optional.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(level, piglin, optional.get())) {
                return optional;
            } else {
                ...
                if (optional3.isPresent()) {
                    ...
                    return optional2.isPresent() && Sensor.isEntityAttackable(level, piglin, optional2.get()) ? optional2 : Optional.empty();
                }
            }
        }
    }
    ...
    public static void angerNearbyPiglins(ServerLevel level, Player player, boolean requireLineOfSight) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !requireLineOfSight || BehaviorUtils.canSee(p_34881_, player)).forEach(p_375921_ -> {
            if (level.getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(level, p_375921_, player);
            } else {
                setAngerTarget(level, p_375921_, player);
            }
        });
    }
    ...
    public static InteractionResult mobInteract(ServerLevel level, Piglin piglin, Player player, InteractionHand hand) {
        ...
        if (canAdmire(piglin, itemstack)) {
            ...
            holdInOffhand(level, piglin, itemstack1);
            admireGoldItem(piglin);
            stopWalking(piglin);
    ...
    protected static void wasHurtBy(ServerLevel level, Piglin piglin, LivingEntity entity) {
        if (!(entity instanceof Piglin)) {
            ...
            getAvoidTarget(piglin).ifPresent(p_375916_ -> {
                if (p_375916_.getType() != entity.getType()) {
    ...
    private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
        ...
        return CopyMemoryWithExpiry.create(
            p_381525_ -> p_381525_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_381525_.level().random),
    ...
    protected static void broadcastAngerTarget(ServerLevel level, AbstractPiglin piglin, LivingEntity angerTarget) {
        getAdultPiglins(piglin).forEach(p_375906_ -> {
            if (angerTarget.getType() != EntityType.HOGLIN || p_375906_.canHunt() && ((Hoglin)angerTarget).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(level, p_375906_, angerTarget);
            }
        });
    }
    ...
    protected static void broadcastUniversalAnger(ServerLevel level, AbstractPiglin piglin) {
        getAdultPiglins(piglin)
            .forEach(p_375913_ -> getNearestVisibleTargetablePlayer(p_375913_).ifPresent(p_375924_ -> setAngerTarget(level, p_375913_, p_375924_)));
    }
    ...
    private static void setAngerTargetToNearestTargetablePlayerIfFound(ServerLevel level, AbstractPiglin piglin, LivingEntity entity) {
        ...
        if (optional.isPresent()) {
            setAngerTarget(level, piglin, optional.get());
        } else {
            setAngerTarget(level, piglin, entity);
        }
    }
