...
public class AnimalMakeLove extends Behavior<Animal> {
    ...
    private Optional<? extends Animal> findValidBreedPartner(Animal p_animal) {
        return p_animal.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().findClosest(p_381390_ -> {
            if (p_381390_.getType() == this.partnerType && p_381390_ instanceof Animal animal && p_animal.canMate(animal) && !animal.isPanicking()) {
                return true;
            }
