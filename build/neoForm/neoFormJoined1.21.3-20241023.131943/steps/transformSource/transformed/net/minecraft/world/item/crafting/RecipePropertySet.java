...
public class RecipePropertySet {
    ...
    private static ResourceKey<RecipePropertySet> registerVanilla(String name) {
        return ResourceKey.create(TYPE_KEY, ResourceLocation.withDefaultNamespace(name));
    }
    ...
    static RecipePropertySet create(Collection<Ingredient> ingredients) {
        Set<Holder<Item>> set = ingredients.stream().flatMap(p_379762_ -> p_379762_.items().stream()).collect(Collectors.toUnmodifiableSet());
