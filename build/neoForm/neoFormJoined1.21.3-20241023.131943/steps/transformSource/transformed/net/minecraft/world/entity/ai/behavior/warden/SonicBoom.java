...
public class SonicBoom extends Behavior<Warden> {
    ...
    protected void tick(ServerLevel level, Warden owner, long gameTime) {
        owner.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).ifPresent(p_381450_ -> owner.getLookControl().setLookAt(p_381450_.position()));
        if (!owner.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY)
            ...
            owner.getBrain()
                ...
                .ifPresent(p_375715_ -> {
                    ...
                    Vec3 vec31 = p_375715_.getEyePosition().subtract(vec3);
                    ...
                    if (p_375715_.hurtServer(level, level.damageSources().sonicBoom(owner), 10.0F)) {
                        double d1 = 0.5 * (1.0 - p_375715_.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
                        double d0 = 2.5 * (1.0 - p_375715_.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
                        p_375715_.push(vec32.x() * d0, vec32.y() * d1, vec32.z() * d0);
                    }
                });
        }
    }
