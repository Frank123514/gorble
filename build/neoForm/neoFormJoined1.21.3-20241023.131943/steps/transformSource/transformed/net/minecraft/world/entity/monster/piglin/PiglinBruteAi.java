...
public class PiglinBruteAi {
    ...
    private static void initFightActivity(PiglinBrute piglinBrute, Brain<PiglinBrute> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(
            ...
            10,
            ImmutableList.of(
                StopAttackingIfTargetInvalid.create((p_375926_, p_375927_) -> !isNearestValidAttackTarget(p_375926_, piglinBrute, p_375927_)),
    ...
    private static boolean isNearestValidAttackTarget(ServerLevel level, AbstractPiglin piglin, LivingEntity entity) {
        return findNearestValidAttackTarget(level, piglin).filter(p_35085_ -> p_35085_ == entity).isPresent();
    }
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel level, AbstractPiglin piglin) {
        ...
        if (optional.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(level, piglin, optional.get())) {
            ...
            Optional<? extends LivingEntity> optional1 = piglin.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER);
