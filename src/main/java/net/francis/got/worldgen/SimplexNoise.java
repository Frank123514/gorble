...
public final class SimplexNoise {
    ...
    private static final SimplexNoise DEFAULT = new SimplexNoise(0L, false);

    private final short[] perm      = new short[512];
    private final short[] permMod12 = new short[512];

    private SimplexNoise(long seed, boolean seeded) {
        short[] base = {
        ...
        if (seeded) {
            ...
            for (int i = 255; i > 0; i--) {
                ...
                short tmp = base[i]; base[i] = base[j]; base[j] = tmp;
            }
        }
    ...
    public static SimplexNoise seeded(long seed) {
    ...
    public double eval(double xin, double yin) {
        ...
        double t0 = 0.5 - x0*x0 - y0*y0; if (t0 > 0) { t0 *= t0; n0 = t0*t0*dot(GRAD3[gi0], x0, y0); }
        double t1 = 0.5 - x1*x1 - y1*y1; if (t1 > 0) { t1 *= t1; n1 = t1*t1*dot(GRAD3[gi1], x1, y1); }
        double t2 = 0.5 - x2*x2 - y2*y2; if (t2 > 0) { t2 *= t2; n2 = t2*t2*dot(GRAD3[gi2], x2, y2); }
    ...
    private static double dot(double[] g, double x, double y) { return g[0]*x + g[1]*y; }
