...
public class GiantEntity extends PathfinderMob {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            ...
            boolean onMammoth = this.getVehicle() instanceof MammothEntity;
    ...
    public static boolean checkSpawnRules(EntityType<GiantEntity> type,
    ...
    private static class RideMammothGoal extends Goal {
        ...
        @Nullable private MammothEntity targetMammoth;

        RideMammothGoal(GiantEntity giant) {
            this.giant = giant;
        ...
        public void tick() {
            ...
            if (gap <= 2.0) {
                ...
                // TODO(port-1.21.11): Mob#startRiding now takes a 3rd boolean param in this NeoForge build.
                // Compiling with `true` for both flags (force=true) to unblock the build — verify against
                // Mob.java source (or NeoForge primer) what the 2nd/3rd booleans actually control before shipping.
                if (giant.startRiding(targetMammoth, true, true)) {
                    giant.mountAnimTicks = GiantEntity.MOUNT_HOLD_TICKS_PUBLIC;
        ...
        @Nullable
        private MammothEntity findNearestFreeMammoth() {
            double radius = MammothEntity.MOUNT_SEEK_RADIUS;
            List<MammothEntity> nearby = giant.level().getEntitiesOfClass(
                    MammothEntity.class,
            ...
            MammothEntity closest = null;
            double bestDist = Double.MAX_VALUE;
            for (MammothEntity m : nearby) {
