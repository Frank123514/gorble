package net.got.event.entity.npc.levy.arryn;
...
/** Tier-2 Levy conscript of House Arryn (Vale). */
public class ArrynLevyEntity extends LevyEntity {
    ...
    private static ResourceLocation[] textures(boolean female) {
        ...
        for (int i = 0; i < arr.length; i++) {
            arr[i] = ResourceLocation.fromNamespaceAndPath("got",
                    "textures/entity/npc/smallfolk/valeman/generated/" + prefix + "_" + String.format("%02d", i + 1) + ".png");
        }
        return arr;
    }
    ...
    public ArrynLevyEntity(EntityType<? extends ArrynLevyEntity> type, Level level) {
        super(type, level);
    }

    @Override protected GotGenderProvider getGenderProvider() { return GotGenderProvider.MALE; }
    @Override protected GotNameGenerator getNameGenerator() { return GotNpcNames.ARRYN_LEVY; }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty,
        ...
        if (random.nextFloat() < 0.4f) setHelmet(new net.minecraft.world.item.ItemStack(Items.CHAINMAIL_HELMET));
        return result;
    }
