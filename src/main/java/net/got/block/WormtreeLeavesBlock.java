...
public class WormtreeLeavesBlock extends LeavesBlock {
	public static final MapCodec<WormtreeLeavesBlock> CODEC = simpleCodec(WormtreeLeavesBlock::new);

	@Override
	public MapCodec<WormtreeLeavesBlock> codec() { return CODEC; }

    public WormtreeLeavesBlock(Properties p) { super(0.1f, p.sound(SoundType.GRASS).strength(0.2f).noOcclusion().pushReaction(PushReaction.DESTROY).isRedstoneConductor((a,b,c)->false).ignitedByLava().isSuffocating((a,b,c)->false).isViewBlocking((a,b,c)->false)); }
