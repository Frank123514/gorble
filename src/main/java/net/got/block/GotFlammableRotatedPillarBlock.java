        ...
        if (toolAction == ItemAbilities.AXE_STRIP) {
            ...
            for (String woodType : GotModBlocks.LOGS.keySet()) {
                if (GotModBlocks.LOGS.get(woodType).get() == currentBlock) {
                    var strippedLogDeferred = GotModBlocks.STRIPPED_LOGS.get(woodType);
                    if (strippedLogDeferred != null) {
                        return strippedLogDeferred.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
                    }
                }
            }
            ...
            for (String woodType : GotModBlocks.WOODS.keySet()) {
                if (GotModBlocks.WOODS.get(woodType).get() == currentBlock) {
                    var strippedWoodDeferred = GotModBlocks.STRIPPED_WOODS.get(woodType);
                    if (strippedWoodDeferred != null) {
                        return strippedWoodDeferred.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
                    }
                }
            }
        }
